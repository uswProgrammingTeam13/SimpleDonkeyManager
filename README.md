...
- ✅ **모델 검증**: 학습된 모델로 프레임별 추론 → 실제값과 AI 예측값 비교
...
└──────────────────┬──────────────────────────────────┘
                   │
┌──────────────────▼──────────────────────────────────┐
│  ④ 결과 확인 & 검증 (Result & Validation)           │
│  • 학습 곡선 시각화 (Loss, Accuracy)                │
│  • 모델 성능 분석                                   │
│  • 학습된 모델로 검증 실행 (실제값 vs AI 예측)       │
│  • 프레임별 비교 + 검증 결과 요약                    │
└─────────────────────────────────────────────────────┘
```
...
│   └── ValidationViewer.cs      # 검증 프레임 뷰어 (실제값/예측값 비교)
│
├── 📂 helptexts/                # 도움말 텍스트
│   ├── InitialHelp.txt
│   ├── DataLoadHelp.txt
...
│   └── validate_model.py        # 학습된 모델 검증(추론) 스크립트
│
...
├── 🔷 ValidationResult.cs       # 검증 결과/요약 데이터 모델
...
4. **결과 확인 및 검증**
   ...
   - 학습 곡선 시각화:
     ...
     - **Accuracy 그래프**: 정확도 변화
   - 모델 성능 요약 및 통계 확인
   - **검증 시작** 버튼 → 학습된 모델로 프레임별 추론 수행
     - 우측 검증 패널에 현재 처리 중인 프레임 이미지를 실시간 표시
     - 그래프가 검증 그래프(실제 조향값 vs AI 예측 조향값)로 전환
     - 프레임별 실제값/예측값/오차 비교 (◀ ▶ 재생 버튼 제공)
     - 검증 완료 시 요약 표시: 검증 이미지 수, 평균/최대 조향 오차, 평균 속도 오차, 검증 결과 판정(양호/보통/미흡)
   - **폴더 열기**로 모델이 저장된 폴더 바로 이동
...
**검증 결과**: 프레임별 실제값/예측값 비교 및 요약 (조향/속도 오차, 판정 결과)
...
학습된 모델(`.h5`)을 로드해 프레임별 추론을 수행하고, 실제값과 비교한 결과 및 요약을 JSON으로 출력. 진행 상황을 `[PROGRESS]` 마커로 실시간 전달하며, 한글(비ASCII) 모델 경로는 임시 ASCII 경로로 복사해 TensorFlow 로드 제약을 우회.
